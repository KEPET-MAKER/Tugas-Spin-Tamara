...
function drawWheel() {
  ...
  for (let i = 0; i < names.length; i++) {
    ...
    ctx.drawImage(img, radius - 96, -70, 95, 95);
...
canvas.addEventListener("click", (event) => {
  ...
  if (distance <= 50) {
    if (!isSpin) {
      spinAngle = Math.random() * 2000 + 500; // Kecepatan putaran
      isSpin = true;
      rotateWheel();
    }
  }
...
spin.play();
  ...
  spinAngle *= 0.97 + (Math.random() * 0.02); // Variasi perlambatan yang lebih dramatis
